...
class _LookingForWidgetState extends State<LookingForWidget> {
  ...
  final List<Map<String, String>> availableOptions = [
    {'name': 'Langetermijn relatie', 'emoji': '💑'},
    {'name': 'Iets serieus', 'emoji': '❤️'},
    {'name': 'Iets casuals', 'emoji': '😊'},
    {'name': 'Alleen plezier', 'emoji': '🎉'},
    {'name': 'Nieuwe vrienden', 'emoji': '👥'},
    {'name': 'Weet ik nog niet', 'emoji': '🤷‍♀️'},
  ];
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          selectedOption = profile?.lookingFor;
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  Future<void> _updateLookingFor(String option) async {
    final value = option.isEmpty ? null : option;
    setState(() => selectedOption = value);

    print('Looking For Widget: Updating to: $value');

    try {
      await _profileService.updateField('looking_for', value);
      print('Looking For Widget: Successfully updated!');
    } catch (e) {
      print('Looking For Widget: Error updating: $e');
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        GestureDetector(
          onTap: () {
            _showLookingForSelection(context);
          },
          child: Container(
  ...
  void _showLookingForSelection(BuildContext context) {
